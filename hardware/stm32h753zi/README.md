...
3. **PROMPT H3 (committed)** — close the loop: send a `TxSignal`
...
4. **PROMPT C3 (committed)** — the integration glue (init chain,
   SysTick scheduler, EcuM/BswM-lite startup, App seam) is now
   generated by `backend/gen/ecu_glue.py`. A real project's only
   hand-written code is `App_Init()` / `App_MainFunction()`. The
   `src/main.c` from H3 stays around as a debugging reference,
   reachable via `make APP_VARIANT=manual`.

5. **PROMPT C4 (this commit)** — OpenVinci can now assemble a
   COMPLETE, self-contained STM32H7 firmware project (BSW + CMSIS
   + generated configs + glue + Makefile + README) and serve it via
   `/api/generate/zip`. Run `python -m gen.project_export
   h7-loopback --output /tmp/export` and the resulting folder
   builds with `arm-none-eabi-gcc` alone — no references back to
   this repo. CI verifies that on every push.
...
- `src/board.c` / `include/board.h` — board-specific helpers
  (USART3 VCP init, `board_vcp_puts`, `board_vcp_put_hex8`,
  newlib-nano `_init` / `_fini` stubs). One place to change the
  baud or pins per board.
- `generated/EcuM.{h,c}` — startup orchestration + `int main(void)`
  (PROMPT C3). Runs the BSW init chain in order, brings the CAN
  controllers up, starts every Com I-PDU group, then hands control
  to `App_Init()` and `Sched_Init()` and idles in `WFI`.
- `generated/Sched.{h,c}` — SysTick handler pumping
  `Can/CanIf/Com_MainFunction*` + `App_MainFunction` every 1 ms.
  Tick period configurable in the generator (`tick_period_ms`).
- `generated/App.h` + `generated/App_Demo.c` — the application seam.
  The .h declares `App_Init() / App_MainFunction()`; the .c is the
  REPLACE-ME demo that drives the loopback test.
- `src/main.c` — the original manual variant (everything from H3).
  Built with `make APP_VARIANT=manual`; kept as a debugging
  reference, not what real projects use.
...
make                    # → build/openvinci-h7.{elf,bin,hex}
                        #   (default APP_VARIANT=generated)
make APP_VARIANT=manual # → same binary, original src/main.c entry
                        #   (the H3 "everything in one file" variant)
